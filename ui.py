...
def extract_video_embedding(video_bytes: bytes, filename: str) -> np.ndarray:
    """
    Since OpenFace features are not available in runtime,
    return a neutral zero vector so the classifier
    relies on text + audio only.
    """
...
if analyse_clicked:
    ...
    # ── Early fusion
    combined = np.concatenate([text_feat, audio_feat, video_feat]).astype(np.float32)

    # Normalize vector to prevent feature magnitude bias
